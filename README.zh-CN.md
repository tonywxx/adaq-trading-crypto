...
- **`signing`** —— 一个共享的深层模块（[`src/signing.rs`](src/signing.rs)，对应 ADR-0013 的 `sign` 接缝），收口所有与交易所无关的签名原语——HMAC（SHA-256/384/512，hex 与 base64）、可恢复 ECDSA（Polymarket EIP-712）、RSA-PSS（Kalshi）——、凭据抽取（`require_api_key` / `require_secret` / `require_passphrase`），以及无 panic 的请求头装配（`set_header`）。各适配器的 `private_request` 只需保留交易所专属的认证串拼接与请求头名集。
...
> **v1.0.5 新增 —— 统一签名整合（无接口面、无交易所变更）**：ADR-0013 的 `sign` 接缝现收口为单一共享深层模块。`src/signing.rs` 把全部与交易所无关的签名原语——HMAC（SHA-256/384/512）、可恢复 ECDSA（Polymarket EIP-712）、RSA-PSS（Kalshi）——集中到同一可测面之下。ECDSA 签名从 `eip712.rs` 迁出（该模块现仅保留 EIP-712 摘要/编码），RSA-PSS 从 `kalshi.rs` 迁出，Kraken 内联的 HMAC-SHA512 现复用共享的 `hmac_sha512_b64_bytes` 字节密钥变体。发布工作流现会在所有发布重试失败时报错退出。行为与差分 fixture 均保持不变。
