//! okxus 适配器(ADR-0017,由 generated promote 为 curated)。
//!
//! 手写完整交易 API。OKX US(v5)签名与 okx 一致:`base64(HMAC-SHA256(
//! secret, timestamp+method+requestPath+body))` + `OK-ACCESS-*` 头。
//! base URL 取 `https://www.okx.us/api/v5`(OKX 美国站),以官方文档为准。
...
pub const ID: &str = "okxus";
const BASE_URL: &str = "https://www.okx.us/api/v5";
...
impl OkxUs {
    ...
    pub fn new(config: Config) -> Result<Self> {
        let core = HttpCore::new(&config, BASE_URL, RATE_LIMIT_MS, "okxus")?;
        Ok(Self { config, core })
    }
    ...
    fn sign_str(&self, timestamp: &str, method: &str, path: &str, body: &str) -> Result<String> {
        let secret = self
            ...
            .secret
            .as_deref()
            .ok_or_else(|| Error::new(ErrorKind::Authentication, "okxus secret required"))?;
    ...
    async fn private_request(
        ...
        let api_key = self
            ...
            .api_key
            .as_deref()
            .ok_or_else(|| Error::new(ErrorKind::Authentication, "okxus api_key required"))?;
        let passphrase = self
            .config
            .password
            .as_deref()
            .ok_or_else(|| Error::new(ErrorKind::Authentication, "okxus password required"))?;
