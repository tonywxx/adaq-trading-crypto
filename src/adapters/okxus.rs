...
//!
//! OKX 美国站(`https://www.okx.us/api/v5`)。与 okx 主站仅 `id` 与 base URL 不同,
//! 签名(`OK-ACCESS-*` + HMAC-SHA256)、解析、市集逻辑完全复用 okx 主实现
//! (候选 ③ 参数化:删除原 ~570 行近逐字克隆,只剩薄包装 + 字段转发)。
...
/// okxus = okx 美国站实例(id 与 base URL 不同,实现复用)。
pub struct OkxUs {
    inner: Okx,
}
...
impl OkxUs {
    ...
    pub fn new(config: Config) -> Result<Self> {
        Okx::with_endpoints(config, "okxus", "https://www.okx.us/api/v5", 110).map(|inner| OkxUs { inner })
    }
}
