...
mod coinbase_diff {
    ...
    fn setup() -> Coinbase {
        let mut config = Config::new();
        config.enable_rate_limit = false;
        Coinbase::new(config).expect("coinbase adapter")
    }
    ...
    fn markets_match_ccxt() {
        let ex = setup();
        let raw = load_raw("coinbase", "markets");
        let ccxt_fixture = load_ccxt_parsed("coinbase", "markets");
        let ccxt = parsed(&ccxt_fixture);
        let raw_arr = raw["raw"].as_array().unwrap();
        let ccxt_arr = ccxt.as_array().unwrap();
        assert_eq!(raw_arr.len(), ccxt_arr.len());
    ...
    fn ticker_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("coinbase", "ticker");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("coinbase", "ticker");
        let ccxt = parsed(&ccxt_fixture);
        // 复刻 fetch_ticker_v3:trades[0] + best_bid/best_ask
        let first = raw["trades"]
            .as_array()
            .and_then(|a| a.first())
            .cloned()
            .unwrap_or_default();
        let mut t = ex.parse_ticker(&first);
        t.bid = raw.get("best_bid").and_then(super::dec).or(t.bid);
        t.ask = raw.get("best_ask").and_then(super::dec).or(t.ask);
        assert_eq!(
            t.symbol,
            obj_str(ccxt, "symbol").unwrap_or_default(),
            "ticker.symbol"
        ...
        assert_eq!(t.datetime, obj_str(ccxt, "datetime"), "ticker.datetime");
    ...
    fn order_book_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("coinbase", "order_book");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("coinbase", "order_book");
        let ccxt = parsed(&ccxt_fixture);
        let book = ex.parse_order_book(&raw, "BTC/USDT");
        assert_eq!(
            book.bids.len(),
    ...
    fn trades_match_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("coinbase", "trades");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("coinbase", "trades"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
    ...
    fn ohlcv_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("coinbase", "ohlcv");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("coinbase", "ohlcv"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
...
mod gate_diff {
    ...
    fn setup() -> Gate {
        let mut config = Config::new();
        config.enable_rate_limit = false;
        Gate::new(config).expect("gate adapter")
    }
    ...
    fn markets_match_ccxt() {
        let ex = setup();
        let raw = load_raw("gate", "markets");
        let ccxt_fixture = load_ccxt_parsed("gate", "markets");
        let ccxt = parsed(&ccxt_fixture);
        let raw_arr = raw["raw"].as_array().unwrap();
        let ccxt_arr = ccxt.as_array().unwrap();
        assert_eq!(raw_arr.len(), ccxt_arr.len());
    ...
    fn ticker_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("gate", "ticker");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("gate", "ticker");
        let ccxt = parsed(&ccxt_fixture);
        let t = ex.parse_ticker(&raw);
        assert_eq!(
            t.symbol,
    ...
    fn order_book_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("gate", "order_book");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("gate", "order_book");
        let ccxt = parsed(&ccxt_fixture);
        let book = ex.parse_order_book(&raw, "BTC/USDT");
        assert_eq!(
            book.bids.len(),
    ...
    fn trades_match_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("gate", "trades");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("gate", "trades"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
    ...
    fn ohlcv_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("gate", "ohlcv");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("gate", "ohlcv"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
...
mod mexc_diff {
    ...
    fn setup() -> Mexc {
        let mut config = Config::new();
        config.enable_rate_limit = false;
        Mexc::new(config).expect("mexc adapter")
    }
    ...
    fn markets_match_ccxt() {
        let ex = setup();
        let raw = load_raw("mexc", "markets");
        let ccxt_fixture = load_ccxt_parsed("mexc", "markets");
        let ccxt = parsed(&ccxt_fixture);
        let raw_arr = raw["raw"].as_array().unwrap();
        let ccxt_arr = ccxt.as_array().unwrap();
        assert_eq!(raw_arr.len(), ccxt_arr.len());
    ...
    fn ticker_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("mexc", "ticker");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("mexc", "ticker");
        let ccxt = parsed(&ccxt_fixture);
        let t = ex.parse_ticker(&raw);
        assert_eq!(
            t.symbol,
        ...
        // average:ccxt 按市场价格精度(float)舍入,我们用精确 decimal → 宽松容差
        match (t.average, obj_dec(ccxt, "average")) {
            (Some(a), Some(b)) => {
                let diff = (a - b).abs();
                let scale = a.abs().max(b.abs());
                assert!(
                    diff.is_zero() || (scale > Decimal::ZERO && diff / scale <= Decimal::new(1, 6)),
                    "ticker.average: ours={a} ccxt={b}"
                );
            }
            (None, None) => {}
            (a, b) => panic!("ticker.average: ours={a:?} ccxt={b:?}"),
        }
    }
    ...
    fn order_book_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("mexc", "order_book");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("mexc", "order_book");
        let ccxt = parsed(&ccxt_fixture);
        let book = ex.parse_order_book(&raw, "BTCUSDT");
        assert_eq!(
            book.bids.len(),
            ccxt["bids"].as_array().unwrap().len(),
            "bids 数量"
    ...
    fn trades_match_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("mexc", "trades");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("mexc", "trades"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
    ...
    fn ohlcv_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("mexc", "ohlcv");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("mexc", "ohlcv"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
...
mod htx_diff {
    ...
    fn setup() -> Htx {
        let mut config = Config::new();
        config.enable_rate_limit = false;
        Htx::new(config).expect("htx adapter")
    }
    ...
    fn markets_match_ccxt() {
        let ex = setup();
        let raw = load_raw("htx", "markets");
        let ccxt_fixture = load_ccxt_parsed("htx", "markets");
        let ccxt = parsed(&ccxt_fixture);
        let raw_arr = raw["raw"].as_array().unwrap();
        let ccxt_arr = ccxt.as_array().unwrap();
        assert_eq!(raw_arr.len(), ccxt_arr.len());
    ...
    fn ticker_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("htx", "ticker");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("htx", "ticker");
        let ccxt = parsed(&ccxt_fixture);
        let mut t = ex.parse_ticker(&raw);
        // merged tick 无 symbol 字段(由请求推导)→ 用 id 回退解析(与 kraken 同法)
        t.symbol = ex.market_symbol("btcusdt");
        assert_eq!(
            t.symbol,
            obj_str(ccxt, "symbol").unwrap_or_default(),
            "ticker.symbol"
    ...
    fn order_book_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("htx", "order_book");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("htx", "order_book");
        let ccxt = parsed(&ccxt_fixture);
        let book = ex.parse_order_book(&raw, "BTC/USDT");
        assert_eq!(
            book.bids.len(),
    ...
    fn trades_match_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("htx", "trades");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("htx", "trades"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
    ...
    fn ohlcv_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("htx", "ohlcv");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("htx", "ohlcv"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
...
mod kucoin_diff {
    ...
    fn setup() -> Kucoin {
        let mut config = Config::new();
        config.enable_rate_limit = false;
        Kucoin::new(config).expect("kucoin adapter")
    }
    ...
    fn markets_match_ccxt() {
        let ex = setup();
        let raw = load_raw("kucoin", "markets");
        let ccxt_fixture = load_ccxt_parsed("kucoin", "markets");
        let ccxt = parsed(&ccxt_fixture);
        let raw_arr = raw["raw"].as_array().unwrap();
        let ccxt_arr = ccxt.as_array().unwrap();
        assert_eq!(raw_arr.len(), ccxt_arr.len());
    ...
    fn ticker_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("kucoin", "ticker");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("kucoin", "ticker");
        let ccxt = parsed(&ccxt_fixture);
        let t = ex.parse_ticker(&raw);
        assert_eq!(
            t.symbol,
    ...
    fn order_book_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("kucoin", "order_book");
        let raw = raw_fixture["raw"].clone();
        let ccxt_fixture = load_ccxt_parsed("kucoin", "order_book");
        let ccxt = parsed(&ccxt_fixture);
        let book = ex.parse_order_book(&raw, "BTC/USDT");
        assert_eq!(
            book.bids.len(),
    ...
    fn trades_match_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("kucoin", "trades");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("kucoin", "trades"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
        for (i, (r, c)) in raw_arr.iter().zip(ccxt.iter()).enumerate() {
            let t = ex.parse_trade(r);
            assert_eq!(t.id.as_deref(), c["id"].as_str(), "trade[{i}].id");
            // 纳秒→毫秒换算经 ccxt float 运算有 ±2ms 抖动
            let ours_ts = t.timestamp;
            let theirs_ts = c["timestamp"].as_i64();
            match (ours_ts, theirs_ts) {
                (Some(a), Some(b)) => assert!(
                    (a - b).abs() <= 2,
                    "trade[{i}].timestamp: ours={a} ccxt={b}"
                ),
                (None, None) => {}
                (a, b) => panic!("trade[{i}].timestamp: ours={a:?} ccxt={b:?}"),
            }
            assert_eq!(
                t.side.as_deref(),
                obj_str(c, "side").as_deref(),
    ...
    fn ohlcv_matches_ccxt() {
        let ex = setup();
        let raw_fixture = load_raw("kucoin", "ohlcv");
        let raw_arr = raw_fixture["raw"].as_array().unwrap().clone();
        let ccxt = parsed(&load_ccxt_parsed("kucoin", "ohlcv"))
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(raw_arr.len(), ccxt.len(), "行数一致");
